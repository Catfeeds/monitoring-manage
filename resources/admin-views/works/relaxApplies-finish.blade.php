...
@section('content')
    @include('admin::search.relaxApplies-finish')

    <div class="row">
        <div class="col-md-12">
            <div class="box box-info">
                <div class="box-header with-border">
                    <h3 class="box-title">请假列表-已完成</h3>

                    @include('admin::widgets.filter-btn-group', ['resetUrl' => route('admin::relaxApplies.finish')])
                </div>
                    ...
                    <table class="table">
                        ...
                        @foreach($relaxApplies as $relaxApply)
                            <tr>
                                ...
                                <td>{{ $relaxApply->date_num }}</td>
                                <td>{{ $relaxApply->teacher->name }}</td>
                                <td>{!! $relaxPresenter->status($relaxApply)!!}</td>
                                <td>{{ $relaxApply->created_at }}</td>
                                <td>
                                    <a href="javascript:void(0);" data-id="{{ $relaxApply->id }}" style="padding:3px 6px;" class="btn btn-danger btn-sm grid-row-delete" role="button">
...
@section('script')
    @include('admin::js.grid-row-delete', ['url' => url("/admin/relaxApplies")])

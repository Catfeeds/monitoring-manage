...
class ArticleController extends Controller
{
    ...
    {
        $articles = (new Article())->where('school_id','=',auth('teacher')->user()->school->id)->orderBy('created_at','desc')->get();
    ...
    public function store(Request $request){
        ...
        if($user) {
            ...
            $article->school_id = auth('teacher')->user()->school->id;
...
//        if(auth('teacher')->user()->can('delete',$article)) {
//            $article->delete();
...
//    }
    ...
    public function zan(Article $article){
        $user = auth('teacher')->user();
        if($user) {
            $zan = new Zan(['user_id'=>$user->id,'type'=>1]);
    ...
    public function comment(Article $article,Request $request){
        ...
        if($user && $request->get('content')) {
            $comment = new Comment(['user_id'=>$user->id,'content'=>$request->get('content'),'type'=>1]);
...
//    }
    ...
    public function addlabel(Request $request){
        if(auth('teacher')->user()->labels->count()<=8) {
            UserLabel::create(['name' => $request->get('name'), 'user_id' => auth('teacher')->user()->id,'type'=>1]);
